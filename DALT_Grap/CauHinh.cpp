...
CauHinh::CauHinh(char* MaSP, char* MaModel, char* ManHinh, char* CPU, char* GPU, char* RAM, char* LuuTru, int Pin_Wh, float KhoiLuong_Kg, int Gia)
{
	...
	strcpy_s(this->ManHinh, 15, ManHinh);
	strcpy_s(this->CPU, 25, CPU);
	strcpy_s(this->GPU, 50, GPU);
	strcpy_s(this->RAM, 15, RAM);
	strcpy_s(this->LuuTru, 25, LuuTru);
	this->Pin_Wh = Pin_Wh;
...
char* CauHinh::getMaSP() {
	char* MaSP = this->MaSP;
	for (int i = 0; i < strlen(MaSP); i++) {
		if (MaSP[i] == ' ') {
			MaSP[i] = '\0';
			break;
		}
